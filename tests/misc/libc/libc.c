...
u8 __attribute__((aligned(16))) data1[1024], data2[1024];
...
void dumpData(const char *title, u8 *data) {
	printf("%s:\t", title);
	int i;
	for (i = 0; i < 0xFF; ++i)
		printf("%02x", data[i]);
...
void testCopy() {
	...
	int i = 0;
	for (i = 0; i < 0xFF; ++i)
		sce_paf_private_memset(data1 + i, i, 1);

	// If not aligned to 16, weird things can happen, but replicating that will be hard.
	dumpData("Before", data1);
	result = sce_paf_private_memcpy(data1 + 16, data1, 240);
	dumpData("After", data1);
